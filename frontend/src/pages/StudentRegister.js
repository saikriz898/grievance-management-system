  ...
  const sendRegisterOTP = async () => {
    ...
    try {
      const response = await api.post('/otp/send', { 
        ...
        type: 'register',
        message: `Welcome to GrievAI! Your registration verification code is: {otp}. Valid for 5 minutes.`
      });
            ...
            {step === 1 && (
              <>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-2xl transition-all shadow-xl hover:shadow-2xl transform hover:-translate-y-1 disabled:opacity-50 disabled:transform-none flex items-center justify-center"
                >
                  {loading ? (
                    <>
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-gray-500">
                    {otpTimer > 0 ? `${Math.floor(otpTimer/60)}:${(otpTimer%60).toString().padStart(2,'0')}` : (
                      <button onClick={sendRegisterOTP} className="text-blue-600 hover:text-blue-800">
                        Resend OTP
                      </button>
                    )}
                  </span>
